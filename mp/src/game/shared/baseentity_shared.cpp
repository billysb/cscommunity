...
void CBaseEntity::ImpactTrace( trace_t *pTrace, int iDamageType, const char *pCustomImpactName )
{
	...
	// Really need to figure out what is passing invalid entities to this. This is a hacky fix that probably hurts fps on the client side.
	// Don't try to impact invalid entities. Attempt not to crash. -BillySB
	if (pEntity == NULL)
	{
		return;
	}
