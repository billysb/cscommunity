...
void CCSPlayer::Spawn()
{
	...
	{
		// Regular speed outside of terror strike.
		SetMaxSpeed(250);
	}
#else
	SetMaxSpeed(250);
	...
	{
		// Model setting moved from RoundRespawn to Spawn to fix any possible chance of them not looking like zombies.

		// Possible zombie models.
		const char* models[] = {
			"models/player/gregrogers/infectedl4d1/male_worker/gregrogers.mdl",
			"models/player/gregrogers/infectedl4d1/male_rural/gregrogers.mdl",
			"models/player/gregrogers/infectedl4d1/male/gregrogers.mdl",
			"models/player/gregrogers/infectedl4d1/female/gregrogers.mdl",
			"models/player/gregrogers/infectedl4d1/male_suit/gregrogers.mdl",
			"models/player/gregrogers/infectedl4d1/male_police/gregrogers.mdl",
			"models/player/gregrogers/infectedl4d1/male_military/gregrogers.mdl",
			"models/player/gregrogers/infectedl4d1/male_patient/gregrogers.mdl"
		};

		// Calculate the number of elements in the array
		int numModels = sizeof(models) / sizeof(models[0]);
		// Generate a random index
		int randomIndex = rand() % numModels;

		PrecacheModel(models[randomIndex]);
...
void CCSPlayer::Event_Killed(const CTakeDamageInfo &info)
{
	...
	if (bIsTerror && terrorstrike_deathalerts.GetBool() && GetTeamNumber() == TEAM_TERRORIST)
	{
		for (int i = 0; i < MAX_PLAYERS; i++)
		{
			...
			if (pPlayer == this)
			{
				// play different sound for ourself.
				engine->ClientCommand(pPlayer->edict(), UTIL_VarArgs("play \"%s\"\n", "music/undeath/LeftForDeath.wav"));
			}
			else
			{
				engine->ClientCommand(pPlayer->edict(), UTIL_VarArgs("play \"%s\"\n", "survivor_die.wav"));
			}
			//pPlayer->EmitPrivateSound("survivor_die.wav");
		}
	}
#endif
}
...
void CCSPlayer::ResetMaxSpeed()
{
	...
	{
		speed = terrorstrike_zombiespeed.GetFloat();
	}
#endif
...
void CCSPlayer::PreThink()
{
	...
	if (CSGameRules()->IsTerrorStrikeMap())
	{
		CWeaponCSBase *wep = GetActiveCSWeapon();
		if (wep != NULL && terrorstrike_infiniteammo.GetBool() && GetTeamNumber() == TEAM_TERRORIST && !wep->HasAnyAmmo())
		{
			this->SetAmmoCount(420, wep->m_iPrimaryAmmoType);
		}
		else if (wep != NULL && GetTeamNumber() == TEAM_CT && Q_strcmp(wep->GetClassname(), "weapon_knife") != 0)
		{
			Msg("Dirty weapon hack. Zombie tried to use a weapon that it shouldnt. Deleted!\n");
			
			// Remove weapon if its not the knife. Zombies shouldnt have this.
			wep->Remove();
		}
	}
	...
	else
	{
		if ( HasSecondaryWeapon() )
		{
			...
			if ( pCSWeapon && pCSWeapon->GetCSWpnData().m_bCanUseWithShield == false )
			 	 return BUY_NOT_ALLOWED; // Value is needed. why didnt we have one before? -BillySB
		}
		...
		// This seems to be undefined. What was this for? -BillySB
...
CBaseEntity	*CCSPlayer::GiveNamedItem( const char *pszName, int iSubType )
{
	...
	if (CSGameRules()->IsTerrorStrikeMap() && GetTeamNumber() == TEAM_CT)
	{
		if (Q_stricmp(pszName, "weapon_knife"))
			return NULL;
		else
			IsTerror = true;
	}
